use opencl3::{
    self,
    command_queue::{
        enqueue_nd_range_kernel, enqueue_read_buffer, enqueue_write_buffer, CommandQueue,
    },
    device::Device,
    event::{wait_for_events, Event},
    kernel::Kernel,
    memory::{Buffer, ClMem, CL_MEM_HOST_WRITE_ONLY, CL_MEM_READ_ONLY, CL_MEM_WRITE_ONLY},
    program::CL_STD_3_0,
    ...
    pub unsafe fn prepare(
...

        {
...

            {
                ...
                queue
                    .enqueue_write_buffer(&mut size_buffer, CL_TRUE, 0, &[gpu_addr_test_count], &[])
                    .unwrap();
            }
